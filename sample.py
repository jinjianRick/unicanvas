...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=-5.,
        help="lambda: aggregation ratio of foreground subject and background scene",
    )
    parser.add_argument(
        "--t_tau",
        type=float,
        default=0.3,
        help="Hyperparameter to determine the interval for cross-attention replacement.",
    )
    parser.add_argument(
        "--b_edit",
        default=False,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="height of the target region",
    )
    parser.add_argument(
        "--scale_y",
        type=int,
        default=0,
        help="weight of the target region",
    )
    ...
    token_foreground = model.cond_stage_model.tokenizer(opt.prompt_foreground, truncation=True, max_length=77, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")["input_ids"]
    foreground_index = torch.nonzero(token_foreground == 49407)[0][1]
    foreground_start_index = torch.nonzero(token_foreground == 49408)[0][1]

    token_background = model.cond_stage_model.tokenizer(opt.prompt_background, truncation=True, max_length=77, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")["input_ids"]
    background_index = torch.nonzero(token_background == 49407)[0][1]

    token_subject = model.cond_stage_model.tokenizer(opt.prompt_subject, truncation=True, max_length=77, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")["input_ids"]
    subject_index = torch.nonzero(token_subject == 49407)[0][1]

    length_custom = foreground_index -1
    length_ldm = background_index - 1

    mask_custom = []
    foreground_list = []
    for token_index in range(foreground_start_index, foreground_start_index+subject_index-1):
        mask_custom.append(token_index-1)
        foreground_list.append(token_index)
    
    mask_ldm = []
    for token_index in range(length_ldm):
        mask_ldm.append(token_index)

    background_list = []
    for token_index in range(1, length_custom+1):
        if token_index not in foreground_list:
            background_list.append(token_index)
    
    tokens = {}
    tokens['1'] = foreground_list
    tokens['2'] = background_list
    ...
    if opt.layout_sup:
        ...
        layouts['1'][(cx - scale_x // 2) // 8 + 1: (cx + scale_x // 2) // 8 - 1, (cy - scale_y // 2) // 8 + 1: (cy + scale_y // 2) // 8 - 1] = 1.
        size_dict['1'] = 1 - (scale_x*scale_y)/np.power(img_size,2)
        #size_dict['1'] = 10

        #background
        layouts['2'] = np.ones((img_size // 8, img_size // 8))
        layouts['2'][(cx - scale_x // 2) // 8 + 1: (cx + scale_x // 2) // 8 - 1, (cy - scale_y // 2) // 8 + 1: (cy + scale_y // 2) // 8 - 1] = 0.
        size_dict['2'] = (scale_x*scale_y)/np.power(img_size,2)
        #size_dict['2'] = 0.1
    ...
    if not opt.from_file:
        prompt_foreground = opt.prompt_foreground
        assert prompt_foreground is not None
        data = [batch_size * [prompt_foreground]]
        if opt.add_prompt is not None:
            data = [batch_size * [prompt_foreground.format(opt.add_prompt)]]
        else:
            data = [batch_size * [prompt_foreground]]

        prompt_background = opt.prompt_background
        data_background = [batch_size * [prompt_background]]
    else:
    ...
    extra_info = {'length_ldm': length_ldm, 'length_custom': length_custom, 'mask_ldm': mask_ldm, 'mask_custom': mask_custom, 'b_edit': opt.b_edit, 'bl_path': opt.bl_path, 'bk_opt':opt.bk_opt, 'cor_dict':opt.cor_dict, 'lambda':opt.agg_lambda, 't_tau':opt.t_tau}
            ...
            with model.ema_scope():
                ...
                for prompts in tqdm(data, desc="data"):
                    ...
                    for n in trange(opt.n_iter, desc="Sampling"):
                        ...
                        c_ldm = {}
                        c_ldm['ldm'] = model.get_learned_conditioning(data_background[prompt_count])
                    ...
                    if not opt.skip_grid:
                        ...
                        if opt.prompt_background is not None:
                            img.save(os.path.join(outpath, f'{prompts[0].replace(" ", "-")}_{opt.prompt_background.replace(" ", "-")}_{opt.scale}_{sampling_method}_{opt.ddim_steps}_{opt.ddim_eta}.png'))
                        else:
