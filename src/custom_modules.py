...
class FrozenCLIPEmbedderWrapper(AbstractEncoder):
    ...
    def __init__(self, modifier_token, version="openai/clip-vit-large-patch14", device=None, max_length=77):
        super().__init__()
    ...
    def add_token(self):
        ...
        if len(self.modifier_token) > 3:
            token_embeds[self.modifier_token_id[-4]] = torch.nn.Parameter(token_embeds[43214], requires_grad=True)
        if len(self.modifier_token) > 4:
            token_embeds[self.modifier_token_id[-5]] = torch.nn.Parameter(token_embeds[43014], requires_grad=True)
        if len(self.modifier_token) > 5:
            token_embeds[self.modifier_token_id[-6]] = torch.nn.Parameter(token_embeds[42514], requires_grad=True)
    ...
    def forward(self, text, prompt_dict=None, index=None, device=None, **kwargs):
        #print('text', text)
        self.device = device
        ...
        indices = (indices*1).unsqueeze(-1)       #指示被替代的词在哪里

        input_shape = tokens.size()
        tokens = tokens.view(-1, input_shape[-1])


        hidden_states = self.transformer.text_model.embeddings(input_ids=tokens)    #torch.Size([2, 77, 768])
        #hidden_states2 = self.transformer.text_model.embeddings.token_embedding(tokens)

        if prompt_dict is not None:
            if prompt_dict['per_token']:
                flag = indices.squeeze(-1).sum(dim=1)
                prompts = []
                hidden_states_tmp = hidden_states[:, 1:1+prompt_dict['task_token'], :]
                for i in range(hidden_states.shape[0]):
                    #print('kkl', prompt_dict['c_type'])
                    if flag[i] == 0 or prompt_dict['c_type'][i]=='background':
                        prompts.append(hidden_states[i].unsqueeze(0))
                        continue
                    #print('text', text)
                    if prompt_dict['end_token']:
                        prompts.append(torch.cat([prompt_dict['token_prefix'], prompt_dict['ctx'][index[i]], hidden_states_tmp[i], prompt_dict['class_name_embedding'], prompt_dict['end_ctx'][index[i]], prompt_dict['token_suffix']], dim=0).unsqueeze(0))
                        indices[i][(prompt_dict['token_prefix'].shape[0]):(prompt_dict['token_prefix'].shape[0]+prompt_dict['ctx'][index[i]].shape[0]+hidden_states_tmp[i].shape[0])] = 1
                        start = prompt_dict['token_prefix'].shape[0]+prompt_dict['ctx'][index[i]].shape[0]+hidden_states_tmp[i].shape[0]+prompt_dict['class_name_embedding'].shape[0]
                        indices[i][start : (start+prompt_dict['end_ctx'][index[i]].shape[0])] = 1
                    else:
                        prompts.append(torch.cat([prompt_dict['token_prefix'], prompt_dict['ctx'][index[i]], hidden_states_tmp[i], prompt_dict['token_suffix']], dim=0).unsqueeze(0))
                        indices[i][(prompt_dict['token_prefix'].shape[0]):(prompt_dict['token_prefix'].shape[0]+prompt_dict['ctx'][index[i]].shape[0]+hidden_states_tmp[i].shape[0])] = 1
                    
                    #print(indices[i].squeeze())
                    #print('a', prompt_dict['token_prefix'], prompt_dict['ctx'][index[i]], inputs_embeds[i], prompt_dict['token_suffix'])
                

                hidden_states = torch.cat(prompts, dim=0)
