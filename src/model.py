...
class CustomDiffusion(LatentDiffusion):
    def __init__(self,
        ...
        def new_forward(self, x, context=None, mask=None, prompt_dict=None, info_dict=None, latent=None, layer_info=None, layout_info=None):
            ...
            #np.savetxt('./tmp/'+str(crossattn)+str(layout_info is None)+'q.txt', q.view(24,-1).detach().cpu().numpy(), fmt='%f')
            ...
            #if crossattn and prompt_dict['run_type']=='fine_tune' and layout_info is not None:
            if layout_info is not None:
                #sim_ori = sim[int(sim.size(0)/4):int(sim.size(0)/2)][:, :, 1:1+info_dict['length_custom']]
                ...
                if crossattn:
                    ...
                    if layout_info['timesteps'][0].item()/1000 > 0.7:
                        ...
                        if prompt_dict['run_type']=='inference':
                            if info_dict is not None:
                                ...
                                #min_value = sim[-int(sim.size(0)/4):].min(-1)[0].unsqueeze(-1)
                                #max_value = sim[-int(sim.size(0)/4):].max(-1)[0].unsqueeze(-1)
                            else:
                        ...
                        #print('opop', layout_info['creg_maps'][sim.size(1)].shape)
                        #print('pp2', max_value.shape)
                        #mask = layout_info['creg_maps'][sim.size(1)].repeat(1, self.heads,1,1).view(-1, sim.shape[-2], sim.shape[-1])
                        #print('pppi', max_value)   # torch.Size([24, 4096, 77])
                        #np.savetxt('b1.txt', mask.view(-1, 77).detach().cpu().numpy(), fmt='%d')

                        #size_reg = layout_info['size_reg'][sim.size(1)].repeat(self.heads,1,1)
                        #print('oo', layout_info['size_reg'][sim.size(1)].shape)
                        #print('oo2', layout_info['c_size_reg'][sim.size(1)].shape)
                        #size_reg = torch.tensor(layout_info['c_size_reg'][sim.size(1)]).repeat(1, self.heads,1).view(-1, 77).unsqueeze(-2).to(sim.device)
                        #size_reg = torch.tensor(layout_info['size_reg'][sim.size(1)]).unsqueeze(1).repeat(1, self.heads,1,sim.shape[-1]).view(-1, sim.shape[-2], sim.shape[-1]).to(sim.device)
                        ...
                        #print('oo2', size_reg.shape)
                        #print('treg', treg.shape)
                        #print('size_reg', size_reg)
                        #print('pp6', treg)
                        #print('p3', mask.device)
                        
                        if prompt_dict['run_type']=='inference':
                            if info_dict is not None:
                                #use dci
                                #print('cc', mask)
                                #print('kk', layout_info['timesteps'][0].item())
                                ...
                                #sim[int(sim.size(0)/4):int(sim.size(0)/2)] += (mask>0)*creg*(max_value-sim[int(sim.size(0)/4):int(sim.size(0)/2)])
                                #sim[int(sim.size(0)/4):int(sim.size(0)/2)] += (mask>0)*creg*treg*(max_value-sim[int(sim.size(0)/4):int(sim.size(0)/2)])
                                #sim[int(sim.size(0)/4):int(sim.size(0)/2)] += 2*(mask>0)*treg*creg*sim[int(sim.size(0)/4):int(sim.size(0)/2)]
                                #sim[int(sim.size(0)/4):int(sim.size(0)/2)] = sim[int(sim.size(0)/4):int(sim.size(0)/2)]*(mask>0) + ~(mask>0)*(-1e6)    #只变区域外
                                #sim[int(sim.size(0)/4):int(sim.size(0)/2)] = max_value*(mask>0) + ~(mask>0)*(-1e6)
                                #sim[int(sim.size(0)/4):int(sim.size(0)/2)] += 0.1*(mask>0)*creg*(max_value-sim[int(sim.size(0)/4):int(sim.size(0)/2)])
                                #sim[int(sim.size(0)/4):int(sim.size(0)/2)] = sim[int(sim.size(0)/4):int(sim.size(0)/2)]*(mask>0) + ~(mask>0)*(-1e6) 
                            ...
                            #np.savetxt('./tmp/'+'sim.txt', sim.view(-1,77).detach().cpu().numpy(), fmt='%f')
                            #np.savetxt('./tmp/'+'mask.txt', ((mask>0)*size_reg*creg*treg*(max_value-sim)).view(-1,77).detach().cpu().numpy(), fmt='%f')
                            #np.savetxt('./tmp/'+'mask2.txt', (~(mask>0)*size_reg*creg*treg*(sim-min_value)).view(-1,77).detach().cpu().numpy(), fmt='%f')

                            sim += (mask>0)*size_reg*creg*treg*(max_value-sim)
                            sim -= ~(mask>0)*size_reg*creg*treg*(sim-min_value)
                            #print('mask1', ((mask>0)*size_reg*creg*treg).max())
                            #print('mask11', ((mask>0)*size_reg*creg*treg).mean())
                            #print('mask2', (~(mask>0)*size_reg*creg*treg*(sim-min_value)).max())
                            #print('mask', ((mask>0)*creg*(max_value-sim)).max())
                            #np.savetxt('./tmp/'+'mask.txt', ((mask>0)*creg*(max_value-sim)).view(-1,77).detach().cpu().numpy(), fmt='%f')
                            #sim += (mask>0)*treg*creg*(max_value-sim)
                            #sim += 2*(mask>0)*treg*size_reg*creg*sim
                            #sim += 0.1*(mask>0)*creg*(max_value-sim)
                            #sim = sim*(mask>0) + ~(mask>0)*(-1e6)
                            #sim = max_value*(mask>0)*creg + sim*(mask>0)*(1-creg) + ~(mask>0)*(-1e6)
                            #np.savetxt('./tmp/'+'sim.txt', sim.view(-1,77).detach().cpu().numpy(), fmt='%f')
                    ...
                    mask = layout_info['sreg_maps'][sim.size(1)].repeat(1, self.heads, 1, 1).view(-1, sim.shape[-2], sim.shape[-1])
                    ...
                    if prompt_dict['run_type']=='inference':
                        ...
                        sim += (mask>0)*size_reg*sreg*treg*(max_value-sim)
                        sim -= ~(mask>0)*size_reg*sreg*treg*(sim-min_value) """
            ...
            #print('ll', prompt_dict['re_weight'])

            """ if crossattn:
                if prompt_dict is not None and prompt_dict['re_weight']:
                    weight = torch.ones(attn.shape).to(attn.device)
                    for pos, rew in prompt_dict['weight_tuple']:
                        weight[:, :, pos] = rew
                    #weight[:, :, 1] = 3
                    #weight[:, :, 2] = 3
                    #weight[:, :, 3] = 3
            ...
            if crossattn and prompt_dict['run_type']=='inference' and info_dict is not None:
                #attn_ori = sim_ori.softmax(dim=-1)
                ...
                if not info_dict['sep_custom']:
                    ...
                    #attn_cond1 = (10*attn_ori).softmax(dim=-1)
                    ...
                    #print('opll', layout_mask)

                    #layout_mask = torch.cat([mask[0,:, 1].unsqueeze(0), mask[0,:, 7].unsqueeze(0)]).unsqueeze(1)
                    #attn = attn*layout_mask

                    #print('jj', attn.shape)    # torch.size(2, 24, 4096)
                    ...
                    ''' if layer_info['layer_type']+layer_info['layer_count'] == "out11":
                        out1_tmp = attn[0].unsqueeze(-1)*out1 + attn[1].unsqueeze(-1)*out2
                        print('kkk', layer_info['layer_type']+layer_info['layer_count'])
                    else:
                        out1_tmp = out1 '''
                    ...
                    if str(layer_info['layer_type']+layer_info['layer_count']) == "out11" and False:
                        ...
                        #out1_tmp = 2*out1 + out2
                        #out1 = out1_tmp 
                        #out1_tmp = attn[0].unsqueeze(-1)*out1*b_mask1.unsqueeze(-1) + attn[1].unsqueeze(-1)*out2*b_mask2.unsqueeze(-1)
        ...
        """ #########################################################
        #visual_prompt for text transformer
        self.visual_prompt = False

        n_vctx = 4
        compound_prompts_depth = 9

        self.compound_prompts_text = nn.ParameterList([nn.Parameter(torch.empty(n_vctx, 768), requires_grad=True)
                                                      for _ in range(compound_prompts_depth - 1)])    #
        
        for single_para in self.compound_prompts_text:
            nn.init.normal_(single_para, std=0.02)

        #########################################################
        #visual_prompt for cross attention

        self.atten_prompt = False

        n_ctx_att = 2
        self.compound_prompts_depth_att = 9
        visual_ctx_dim = 768

        self.compound_prompts_text_att = nn.ParameterList([nn.Parameter(torch.empty(n_ctx_att, 512))
                                                      for _ in range(self.compound_prompts_depth_att - 1)])
        
        for single_para in self.compound_prompts_text_att:
            nn.init.normal_(single_para, std=0.02)
        
        single_layer = nn.Linear(ctx_dim, visual_ctx_dim)
        self.compound_prompt_projections = nn.ModuleList([copy.deepcopy(single_layer) for i in range(self.compound_prompts_depth_att - 1)])

        self.visual_deep_prompts = []
        for index, layer in enumerate(self.compound_prompt_projections):
            self.visual_deep_prompts.append(layer(self.compound_prompts_text[index]))

        size_dict = {  #cross attention
            'in_1': [n_ctx_att, visual_ctx_dim],
            'in_2': [n_ctx_att, visual_ctx_dim],
            'in_4': [n_ctx_att, visual_ctx_dim],
            'in_5': [n_ctx_att, visual_ctx_dim],
            'in_7': [n_ctx_att, visual_ctx_dim],
            'in_8': [n_ctx_att, visual_ctx_dim],
            'mid_0': [n_ctx_att, visual_ctx_dim],
            'out_3': [n_ctx_att, visual_ctx_dim],
            'out_4': [n_ctx_att, visual_ctx_dim],
            'out_5': [n_ctx_att, visual_ctx_dim],
            'out_6': [n_ctx_att, visual_ctx_dim],
            'out_7': [n_ctx_att, visual_ctx_dim],
            'out_8': [n_ctx_att, visual_ctx_dim],
            'out_9': [n_ctx_att, visual_ctx_dim],
            'out_10': [n_ctx_att, visual_ctx_dim],
            'out_11': [n_ctx_att, visual_ctx_dim],
        }

        add_list = ['in_1', 'in_2', 'in_4', 'in_5', 'in_7', 'in_8', 'mid_0', 'out_3', 'out_4', 'out_5', 'out_6', 'out_7', 'out_8', 'out_9', 'out_10', 'out_11']

        self.kv_prompt = {}

        for layer in add_list:
            self.kv_prompt[layer] = nn.Parameter(torch.empty(size_dict[layer]), requires_grad=True)

        for single_para in self.kv_prompt:
            nn.init.normal_(self.kv_prompt[single_para], std=0.02) """
