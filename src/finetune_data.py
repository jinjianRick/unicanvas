...
class MaskBase(Dataset):
    def __init__(self,
        ...
        self.templates_origin = templates_origin
    ...
    def __getitem__(self, i):
        ...
        if i > self._length2-1 or self._length2 == 0:
            index = i % self._length1
            ...
            if isinstance(self.reg_caption, str):
                ...
                example["caption"] = example["caption"].replace('dish', 'food dish')
                example["caption"] = example["caption"].replace('dish', 'food dish')
        ...
        if self.layout_sup and self.type == 'concept' and (i > self._length2-1 or self._length2 == 0):
            pass
        else:
        ...
        if i > self._length2-1 or self._length2 == 0:
            ...
            random_scale = np.random.randint(self.size // 4, self.size+1)
            # 这一段是用来构造小图的
            #random_scale = self.size // 3    # fix size
            if self.layout_sup:
                if self.type == 'concept':
                    ...
                    size_y, size_x = image.size
                    #print('aa', size_y)
                    #print('bb', size_x)
                    if size_x > size_y:
                        random_scale_x = random_scale
                        random_scale_y = int(random_scale*size_y/size_x)
                    else:
                        random_scale_x = int(random_scale*size_x/size_y)
                        random_scale_y = random_scale

                    if random_scale_x % 2 == 1:
                        random_scale_x += 1
                    if random_scale_y % 2 == 1:
                        random_scale_y += 1

                    #cx = self.size // 2       # fix pos
                    #cy = self.size // 2
                    cx = np.random.randint(random_scale_x // 2, self.size - random_scale_x // 2 + 1)
                    cy = np.random.randint(random_scale_y // 2, self.size - random_scale_y // 2 + 1)

                    image = image.resize((random_scale_y, random_scale_x), resample=self.interpolation)
                    image = np.array(image).astype(np.uint8)
                    image = (image / 127.5 - 1.0).astype(np.float32)

                    input_image1 = np.zeros((self.size, self.size, 3), dtype=np.float32)
                    input_image1[cx - random_scale_x // 2: cx + random_scale_x // 2, cy - random_scale_y // 2: cy + random_scale_y // 2, :] = image

                    save_image = Image.fromarray((255.*np.clip((input_image1 + 1.0) / 2.0, 0.0, 1.0)).astype(np.uint8))
                    save_image.save('tmp_out.png')

                    example['caption'] = np.random.choice(self.templates_origin).format(self.caption)       #####

                    # prepare mask for each token
                    sp_sz = 64
                    sizereg = 1
                    tokens = self.token
                    #tokens = {'concept':[2,3,4,5], 'background':[1, 6]}

                    layout_dict = {}
                    size_dict = {}

                    layout_dict['concept'] = np.zeros((self.size // 8, self.size // 8))
                    layout_dict['concept'][(cx - random_scale_x // 2) // 8 + 1: (cx + random_scale_x // 2) // 8 - 1, (cy - random_scale_y // 2) // 8 + 1: (cy + random_scale_y // 2) // 8 - 1] = 1.
                    size_dict['concept'] = 1 - (random_scale_x*random_scale_y)/np.power(self.size, 2)

                    layout_dict['background'] = np.ones((self.size // 8, self.size // 8))
                    layout_dict['background'][(cx - random_scale_x // 2) // 8 + 1: (cx + random_scale_x // 2) // 8 - 1, (cy - random_scale_y // 2) // 8 + 1: (cy + random_scale_y // 2) // 8 - 1] = 0.
                    size_dict['background'] =  (random_scale_x*random_scale_y)/np.power(self.size, 2)


                    pww_maps = torch.zeros(77,sp_sz,sp_sz)
                    token_size_reg = torch.zeros(1,77)

                    for key in tokens.keys():
                        pos_list = tokens[key]
                        for pos in pos_list:
                            pww_maps[pos, :, :] = torch.tensor(layout_dict[key])
                            token_size_reg[:, pos] = size_dict[key]

                    #l_task_specific = len(self.caption.split(' ')) + 1       ####!!!!
                    #pww_maps[:,1+self.prefix_length:1+self.prefix_length+l_task_specific,:,:] = torch.tensor(layouts)

                    layout_all = []
                    for key in layout_dict.keys():
                        layout_all.append(torch.tensor(layout_dict[key]).unsqueeze(0))
                    ...
                    for r in range(4):
                        ...
                        layouts_s = (layouts_s.view(layouts_s.size(0),1,-1)*layouts_s.view(layouts_s.size(0),-1,1)).sum(0)
                        reg_sizes[np.power(res, 2)] = (1-sizereg*layouts_s.sum(-1, keepdim=True)/(np.power(res, 2))).squeeze(0)
                        #print('op000po', np.power(res, 2), reg_sizes[np.power(res, 2)].shape)
                
                    creg_maps = {}
                    c_size_reg = {}
                    for r in range(4):
                        res = int(sp_sz/np.power(2,r))
                        layout_c = F.interpolate(pww_maps.unsqueeze(0),(res,res),mode='nearest').view(77,-1).permute(1,0) ####!!!!! .repeat(bzs,1,1)
                        creg_maps[np.power(res, 2)] = layout_c
                        c_size_reg[np.power(res, 2)] = token_size_reg

                    layout_info = {}
                    #layout_info['creg_maps'] = creg_maps
                    #layout_info['size_reg'] = reg_sizes
                    #layout_info['c_size_reg'] = c_size_reg
                    #layout_info['creg'] = 1

                    example['creg_maps'] = creg_maps
                    example['size_reg'] = reg_sizes
                    example['c_size_reg'] = c_size_reg
                    example['creg'] = 0.08
                    example['creg'] = 2

                    example['layout_info'] = layout_info

                    mask = np.zeros((self.size // 8, self.size // 8))
                    mask[(cx - random_scale_x // 2) // 8 + 1: (cx + random_scale_x // 2) // 8 - 1, (cy - random_scale_y // 2) // 8 + 1: (cy + random_scale_y // 2) // 8 - 1] = 1.
                    ...
                    mask = np.ones((self.size // 8, self.size // 8))
                    sp_sz = 64
                    pww_maps = torch.ones(77,sp_sz,sp_sz)
                    ...
                    for r in range(4):
                        ...
                        layout_c = F.interpolate(pww_maps.unsqueeze(0),(res,res),mode='nearest').view(77,-1).permute(1,0) ####!!!!! .repeat(bzs,1,1)
                        creg_maps[np.power(res, 2)] = layout_c
                        reg_sizes[np.power(res, 2)] = torch.zeros(np.power(res, 2), 1)
                        c_size_reg[np.power(res, 2)] = torch.zeros(1, 77)
