...
import argparse, os, sys, datetime, glob, json
...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="length of prefix image-specific prompt",
    )
    parser.add_argument(
        "--n_end_ctx",
        type=int,
        default=-1,
        help="length of suffix image-specific prompt",
    )
    parser.add_argument(
        "--add_class",
        default=True,
        help="whether add class name in fine-tuning prompt of reference image",
    )
    parser.add_argument(
        "--per_token",
        default=True,
        help="whether use image-specific prompt,"
    )
    parser.add_argument(
        "--end_token",
        default=True,
        help="wether use suffix image-specific prompt",
    )
    parser.add_argument(
        "--task_token",
        type=int,
        default=2,
        help="length of task-specific content",
    )
    parser.add_argument(
        "--layout_sup",
        default=False,
        help="whether use layout to supervise the fine-tuning",
    )
    parser.add_argument(
        "--save_epoch_interval",
        type=int,
        default=3,
        help="save checkpoint every n checkpoints",
    )
    return parser
...
if __name__ == "__main__":
    ...
    try:
        ...
        if opt.caption2 is not None:
            ...
            config.data.params.train2.params.d_type = opt.type2

        config.data.params.train.params.d_type = opt.type1
        ...
        config.model.params.init_ctx = opt.init_ctx
        config.model.params.ctx_end_init = opt.ctx_end_init

        files = [f for f in os.listdir(opt.datapath) if os.path.isfile(os.path.join(opt.datapath, f))]
        figure_num = len(files)

        config.model.params.figure_num=figure_num
        config.model.params.class_name=opt.class_name.replace('_', ' ')
        config.model.params.n_ctx=opt.n_ctx
        config.model.params.n_end_ctx=opt.n_end_ctx
        config.model.params.add_class=opt.add_class
        config.model.params.per_token=opt.per_token
        config.model.params.end_token=opt.end_token
        config.model.params.task_token=opt.task_token
        config.model.params.run_type='fine_tune'
        config.model.params.layout_sup = opt.layout_sup
        ...
        default_modelckpt_cfg = {
            ...
            #"target": "src.save_check.CustomModelCheckpoint",
            "params": {
                "dirpath": ckptdir,
                "filename": "{epoch:06}",
                "verbose": True,
        ...
        if hasattr(model, "monitor"):
            ...
            default_modelckpt_cfg["params"]["every_n_epochs"] = opt.save_epoch_interval 
        ...
        # data
        if opt.per_token:
            if opt.init_ctx is not None:
                config.data.params.train.params.prefix_length = len(opt.init_ctx.split(' '))
                config.data.params.validation.params.prefix_length = len(opt.init_ctx.split(' '))
            else:
                config.data.params.train.params.prefix_length = opt.n_ctx
                config.data.params.validation.params.prefix_length = opt.n_ctx
        else:
            config.data.params.train.params.prefix_length = 0
            config.data.params.validation.params.prefix_length = 0

        if opt.caption2 is not None:
            config.data.params.train2.params.layout_sup = opt.layout_sup
        
        version="openai/clip-vit-large-patch14"
        tokenizer = CLIPTokenizer.from_pretrained(version)
        prefix_token = tokenizer(opt.init_ctx, truncation=True, max_length=77, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")["input_ids"]
        prefix_index = torch.nonzero(prefix_token == 49407)[0][1] - 1
        suffix_token = tokenizer(opt.ctx_end_init, truncation=True, max_length=77, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")["input_ids"]
        suffix_index = torch.nonzero(suffix_token == 49407)[0][1] - 1
        class_token = tokenizer(opt.class_name, truncation=True, max_length=77, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")["input_ids"]
        class_length = torch.nonzero(suffix_token == 49407)[0][1] - 1

        list_concept = []
        list_background = []
        for token_index in range(prefix_index+1, prefix_index+1+opt.task_token+class_length):
            list_concept.append(token_index)

        for token_index in range(1, prefix_index+1):
            list_background.append(token_index)

        for token_index in range(prefix_index+1+opt.task_token+class_length, prefix_index+1+opt.task_token+class_length+suffix_index):
            list_background.append(token_index)

        token_pos = {}
        token_pos['concept'] = list_concept
        token_pos['background'] = list_background

        config.data.params.train.params.layout_sup = opt.layout_sup
        config.data.params.validation.params.layout_sup = opt.layout_sup
        config.data.params.train.params.tokens = token_pos
        config.data.params.validation.params.tokens = token_pos
